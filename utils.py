...
def readfile(data,undirected=True):
 ...
 #   graphs = pd.read_csv(graph_path, sep=' ', header=None)
    ...
    for edge_type in data:
        for edge in data[edge_type]:
            layerid.append(int(edge_type))
            n1.append(int(edge[0]))
            n2.append(int(edge[1]))
            weight.append(1)
    graphs = {'layerID': layerid, "n1" : n1, "n2":n2,"weight":weight}
    ...
    for layerID, graph in graphs.groupby(['layerID']):
        ...
        layers[layerID] = load_edgelist(new_edges,undirected= undirected)
...
def get_alignment_emb(inter_vectors, layers, id2node):
    ...
    for layerid in layers:
        ...
        for node_id in layers[layerid].keys():
            ...
            tmp[node_id] = inter_vectors[node_id]
...
def get_intra_emb(inter_vectors, W, layers_embedding, layers, id2node):
    ...
    for layerid in layers:
        tmp = {}
        this_layer_embedding = np.dot(inter_vectors, W) + layers_embedding[layerid-1]
        for node in layers[layerid].keys():
            tmp[id2node[node]] = this_layer_embedding[node]
            #print("node_id",node," ", id2node[node],node2id[node])
          #  tmp[node] = this_layer_embedding[node]
        node2vec[layerid] = tmp
